 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of negative and positive numbers`: Unit = {
    assert(sum(List(-1,-3,-5,12)) == 3)
  }

  @Test def `sum of no numbers`: Unit = {
    assert(sum(List()) == 0)
  }
  ...
  @Test def `max of negative and positive numbers`: Unit = {
    assert(max(List(-3, 7, -2)) == 7)
  }
